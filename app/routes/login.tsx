...
export default function Login() {
  ...
  /**
   * Conform Validation.
   * Docs: https://conform.guide
   *
   * Server validation will be skipped in this case, `remix-auth-totp` handles it.
   * Instead we will validate the form on the client.
   */
  const [emailForm, { email }] = useForm({
    onValidate({ formData }) {
      return parse(formData, { schema: LoginEmailFormSchema })
    },
  })
  ...
  return (
    <div className="mx-auto flex h-screen w-screen max-w-7xl flex-col px-6">
      ...
      <div className="mx-auto flex h-full w-full max-w-[350px] flex-col items-center justify-center gap-6">
        ...
        {/* Strategy Errors Handling. */}
        {!authEmail && authError && (
          <span className="font-semibold text-red-400">{authError.message}</span>
        )}
        {/* Conform Errors Handling. */}
        {email.error && <span className="font-semibold text-red-400">{email.error}</span>}
